        ...
        for user, rates in self.rating_data.items():
            ...
            for item, _ in rates.items():
                if item not in self.Q:
                    self.Q[item] = [1.0 * random.random() / math.sqrt(self.F) for x in range(self.F)]
                if item not in self.Y:
                    self.Y[item] = [1.0 * random.random() / math.sqrt(self.F) for j in range(self.F)]
            ...
            for user,rates in self.rating_data.items():
                ...
                for item,rate in rates:
                    for f in range(self.F):
                        z[f]= z[f] + self.Y[item][f]
                ru = 1.0 / math.sqrt(len(rates)*1.0)
                ...
                for item,rate in rates.items():
                    ...
                    for f in range(self.F):
                        s[f] =s[f] + self.Q[item][f] * err_rate
                        self.P[user][f] =self.P[user][f] + self.alpha * (err_rate * self.Q[item][f] - self.lmbd * self.P[user][f])
                        self.Q[item][f] =self.Q[item][f] + self.alpha * (err_rate * self.P[user][f] - self.lmbd * self.Q[item][f])
