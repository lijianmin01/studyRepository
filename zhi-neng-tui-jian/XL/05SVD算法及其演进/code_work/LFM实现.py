...
class LFM:
    ...
    def __init__(self,file_path,F,alpha=0.1,lmbd=0.1,max_iter = 500):
        ...
        for user, rates in self.rating_data.items():
            self.P[user] = [1.0 * random.random() / math.sqrt(self.F) for j in range(self.F)]
    ...
    def train(self):
        lr = self.alpha
        lm = self.lmbd
        for step01 in range(self.max_iter):
            for user,rates in self.rating_data.items():
                for item,rate in rates.items():
                    hat_rate = self.predict(user,item)
                    err_rate = rate-hat_rate
                    for f in range(self.F):
                        self.P[user][f] += lr * (err_rate * self.Q[item][f] - lm * self.P[user][f])
                        self.Q[item][f] += lr * (err_rate * self.P[user][f] - lm * self.Q[item][f])
